...
export class TextureDecoder {
    private static arrayBufferCopy(src, dst, dstByteOffset, numBytes) {
    ...
    public static unpackTextureFormat(available_texture_formats, default_available_texture_formats): number {
        let texture_format;
    ...
    public static unpackImageryEpoch(flags: number, imagery_epoch: number, default_imagery_epoch: number): number {
    ...
    public static unpackUvOffsetAndScale(texture: ITexture, uv_offset_and_scale: number[]) {
        let u_mod = texture.format == 6 ? texture.width : 0xffff + 1;
        let v_mod = texture.format == 6 ? texture.height : 0xffff + 1;
    ...
    public static unpackTexCoords(packed, texture: ITexture, verticesLength): Uint16Array {
    ...
    public static unpackCRN(data, width, height, convertToBMP = true): Uint8Array | Uint16Array {
    ...
    private static dxtToRgb565(src, src16Offset, width, height) {
